...
Artifical radiographs can prepared by run the following:
...
* After training is converaged, one can use `ProGAN.reproduce_trainset.ipynb` to produce artificial radiographs 
...
Inspired by the provious work of [CheXnet](https://stanfordmlgroup.github.io/projects/chexnet/), we trained a DenseNet-121 (ImageNet weights) classifier to evalute the performance of generated radiographs. The code of classifier is contained in folder `Thorax Classifier`.
...
* To obtain the confidience interval, one can use bootstrapping method by run `python -m Analysis.bootstrapping`.
