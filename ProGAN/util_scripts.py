...
def evaluate_metrics(run_id, log, metrics, num_images, real_passes, minibatch_size=None):
    ...
    for title, mode in [('Reals', 'reals'), ('Reals2', 'fakes')][:real_passes]:
        ...
        #labels[:,3] = 1.0
        # Uniform labeling among 14 patholoies
        #cut = int(labels.shape[0] / labels.shape[1])
        #for chunk in range(labels.shape[1]):
            #labels[int(cut*chunk):int(cut*chunk)+cut, chunk] = 1.0 
